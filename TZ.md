...
- собирает текущие цены по магазинам и исторический минимум из внешнего источника (IsThereAnyDeal API);
- накапливает собственную историю цен в виде `PriceSnapshot` при каждом обновлении данных;
- сравнивает текущую цену с историческим минимумом и средней ценой за накопленный период;
...
- `url` — `URLField`, ссылка на страницу покупки в магазине.
...
2. **Карточка игры с аналитикой.** Пользователь открывает игру и видит: цены по всем магазинам, исторический минимум и рассчитанный индекс выгодности (Deal Score) с понятной подписью («отличная цена» / «средне» / «бывало дешевле»).
...
4. **Обновление данных из внешнего источника.** Администратор (или management-команда `update_prices`) запрашивает у IsThereAnyDeal актуальные цены и исторический минимум, создаёт новые `PriceSnapshot`, обновляет `historical_low` у игр.
...
- **Основной сценарий:**
  ...
  2. Для каждой игры из каталога делается запрос к ITAD API (по `itad_id`) для получения текущих цен и исторического минимума.
  3. Ответ парсится, создаются новые `PriceSnapshot`, обновляется `historical_low`; история цен в проекте формируется накоплением этих снимков при повторных обновлениях.
...
- **Внешняя интеграция:** [IsThereAnyDeal API v2](https://docs.isthereanydeal.com/) — получение текущих цен и исторических минимумов по играм. Запросы через `requests`. API-ключ хранится в `.env` (`python-dotenv`), **не** попадает в репозиторий. История цен внутри сервиса формируется из накопленных `PriceSnapshot`.
...
В ходе реализации базовый план был расширен без изменения основной идеи проекта:

- В `PriceSnapshot` добавлено поле `url`, чтобы пользователь мог перейти из каталога или карточки игры напрямую в магазин.
- Добавлен администраторский сценарий поиска игры через ITAD и ручного добавления её в каталог без изменения кода.
- Все цены ITAD отображаются не только в исходных USD, но и в рублях по курсу ЦБ РФ.
- Демо-каталог расширен до 26 игр, для которых API возвращает реальные предложения в официальных магазинах.
