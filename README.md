...
_Добавить после деплоя: главная страница каталога, карточка игры с графиком, личный кабинет._
...
```bash
python manage.py seed_catalog    # создаёт магазины и ~15-18 популярных игр
...
1. На сервере: `git clone <repo-url>`, создать virtualenv с Python 3.10+, `pip install -r requirements.txt`.
2. Создать `.env` на сервере (не коммитится) с `SECRET_KEY`, `DEBUG=False`, `ALLOWED_HOSTS=<your-username>.pythonanywhere.com`, `ITAD_API_KEY`.
...
`# https://...` (будет добавлено после деплоя)
