...
class Command(BaseCommand):
    help = (
        'Загружает текущие цены и исторический минимум из ITAD, '
        'создаёт снимки цен, обновляет флаги в watchlist'
    )
    ...
    def handle(self, *args, **options):
        ...
        if not games:
            self.stdout.write(self.style.WARNING(
                'В каталоге нет игр. Сначала запустите seed_catalog.'
            ))
            return
    ...
    def _update_watchlist_notifications(self):
        for entry in Watchlist.objects.select_related('game'):
            ...
            best_price_usd = (
                PriceSnapshot.objects
                .filter(game=entry.game, recorded_at=latest.recorded_at)
                .order_by('price')
                .values_list('price', flat=True)
                .first()
            )
            best_price_rub = (
                usd_to_rub(best_price_usd)
